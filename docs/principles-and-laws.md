...
Core's `PlaybackRuntime` is the application-scoped owner of the platform player
and active playback coordinator. In this client it is created once, for the
whole application, by `usePlaybackRuntime` (`src/app/usePlaybackRuntime.ts`,
called from `App.tsx`), and the platform player it owns is `ExpoVideoAdapter`
(`src/player/`). Core's `PlaybackCoordinator`, inside it, owns the server
session lease and source generations. React presents snapshots and binds
surfaces; route and presentation changes do not create, replace or destroy
playback resources.
